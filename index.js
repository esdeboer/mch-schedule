...
var data = fs.readFileSync("mch/MCH2022_wiki.ics", "utf8");
...
comp.getAllSubcomponents("vevent").forEach(vevent => {
	...
	if (event.isRecurring()) {
		...
		console.warn("Recurring event" + event)
		return;
	}
...
conference.ele("title", " with self organised sessions");
/*
schedulexml.ele("version", 1);
...
Object.keys(DATES)
	...
	.forEach(function(key) {
		...
		xmlday.att("date", key);
		let startOfDay = DateTime.fromISO(key).set({ hour: 4 });
		xmlday.att("start", startOfDay.toISO({suppressMilliseconds:true}));
		let endOfDay = DateTime.fromISO(key)
			.plus({ day: 1 })
			.set({ hour: 3, minute: 59 });
			xmlday.att("end", endOfDay.toISO({suppressMilliseconds:true}));
		...
		DATES[key]
			...
			.forEach(obj => {
				...
				let xmlroom = rooms[roomName];
				if(!xmlroom){
					xmlroom = xmlday.ele("room");
					xmlroom.att("name", roomName);
					rooms[roomName] = xmlroom;
				}

				var xmlevent = xmlroom.ele("event");
				xmlevent.att(
					"id",
					xmlevent.ele("url", event.uid)
				...
				xmlevent.ele("room", roomName);
				xmlevent.ele("title", event.summary);
				xmlevent.ele("url", event.uid);
				xmlevent.ele("abstract", event.uid);
